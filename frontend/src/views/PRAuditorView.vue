      ...
      <section id="worker-workbench" class="workbench-surface" :aria-label="activeWorkbenchAriaLabel">
        ...
        <div class="workbench-main-grid">
          <div class="workbench-upload-stack">
            ...
            <UploadPanel
              class="cockpit-card upload-card workbench-upload-card"
              title="Generated ECC Upload"
              input-id="pr-auditor-expected-ecc-file"
              input-label="Generated ECC workbook"
              input-hint="Upload the ECC workbook generated by create-pr-cd."
              validate-label="Validate ECC"
              accept=".xlsx,.xlsm"
              :result="prAuditorExpectedEccPrevalidation"
              :loading="prAuditorExpectedEccPrevalidating"
              :disable-action="workerFormLocked"
              @file-selected="onPrAuditorFileSelected('expectedEcc', $event)"
              @prevalidate="prevalidatePrAuditorUpload('expectedEcc', $event)"
            />
          </div>
          ...
          <section class="panel cockpit-card workbench-config-card">
            ...
            <div class="cockpit-empty-card">
              Run Audit uses the validated Final PO and generated ECC workbooks for one dedicated PR Auditor job.
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      prAuditorExpectedEccFile: null,
      prAuditorFinalPoPrevalidation: null,
      prAuditorExpectedEccPrevalidation: null,
      prAuditorFinalPoPrevalidating: false,
      prAuditorExpectedEccPrevalidating: false,
  ...
  computed: {
    ...
    heroSubtitle() {
      return 'Upload Final PO and generated ECC workbooks, run the audit, and review controlled result delivery without exposing PR creation controls.';
    },
    activeWorkerLabel() {
      return 'PR Auditor';
    },
    ...
    canCreateJob() {
      ...
      if (!this.prAuditorFinalPoFile || !this.prAuditorExpectedEccFile) return false;
      if (!this.prAuditorFinalPoPrevalidation || !this.prAuditorFinalPoPrevalidation.passed) return false;
      if (!this.prAuditorExpectedEccPrevalidation || !this.prAuditorExpectedEccPrevalidation.passed) return false;
      return true;
    },
    createDisabledReason() {
      if (!this.prAuditorFinalPoFile || !this.prAuditorExpectedEccFile) {
        return 'Upload Final PO and generated ECC workbooks to start.';
      }
      if (this.prAuditorFinalPoPrevalidating || this.prAuditorExpectedEccPrevalidating) {
        return 'PR Auditor upload validation is in progress.';
      }
      if (!this.prAuditorFinalPoPrevalidation || !this.prAuditorExpectedEccPrevalidation) {
        return 'Validate Final PO and generated ECC workbooks before running the audit.';
      }
      if (!this.prAuditorFinalPoPrevalidation.passed || !this.prAuditorExpectedEccPrevalidation.passed) {
    ...
    consoleItems() {
      const items = [
        {
          ...
          body: 'Upload Final PO and generated ECC workbooks, validate each one, run the audit, then track progress and outputs here.',
          tone: 'info',
          time: ''
        }
      ];
      ...
      if (this.prAuditorExpectedEccFile) {
        items.push({
          id: 'expected-ecc-file-selected',
          label: 'Upload',
          title: 'Generated ECC workbook selected',
          body: this.prAuditorExpectedEccFile.name,
          tone: 'info',
          time: ''
        });
      }
      ...
      if (this.prAuditorExpectedEccPrevalidation) {
        items.push({
          id: 'expected-ecc-validation-state',
          label: 'Validate',
          title: this.prAuditorExpectedEccPrevalidation.passed ? 'Generated ECC validation passed' : 'Generated ECC validation failed',
          body: this.prAuditorExpectedEccPrevalidation.workerExplanation || (this.prAuditorExpectedEccPrevalidation.passed ? 'The generated ECC workbook is ready for audit.' : 'Review the generated ECC validation checklist and correct the workbook.'),
          tone: this.prAuditorExpectedEccPrevalidation.passed ? 'success' : 'danger',
          time: ''
        });
      }
  ...
  methods: {
    ...
    onPrAuditorFileSelected(kind, file) {
      if (kind === 'finalPo') {
        ...
        this.prAuditorExpectedEccFile = file;
        this.prAuditorExpectedEccPrevalidation = null;
      }
    ...
    async prevalidatePrAuditorUpload(kind, file) {
      if (!file) {
        const label = kind === 'finalPo' ? 'Final PO' : 'generated ECC';
      ...
      if (kind === 'finalPo') {
        ...
        this.prAuditorExpectedEccPrevalidating = true;
      }
      this.errorMessage = '';

      const uploadKind = kind === 'finalPo'
        ? 'pr-auditor-final-po'
        : 'pr-auditor-expected-ecc';
      ...
      try {
        ...
        if (kind === 'finalPo') {
          ...
          this.prAuditorExpectedEccPrevalidation = result;
        }
      } catch (error) {
        ...
        if (kind === 'finalPo') {
          ...
          this.prAuditorExpectedEccPrevalidation = fallback;
        }
        ...
        if (kind === 'finalPo') {
          ...
          this.prAuditorExpectedEccPrevalidating = false;
        }
      }
    },
    async createWorkerJob() {
      ...
      try {
        const result = await createJob({
          ...
          expectedEccPrevalidatedFileId: this.prAuditorExpectedEccPrevalidation.prevalidatedFileId
        });
